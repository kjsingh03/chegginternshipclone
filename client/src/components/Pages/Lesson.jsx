  ...
  const getCertificate = () => {
    ...
    const generatePDF = async (name) => {
      ...
      if (userCertificate) {
        ...
        axios.put(`${import.meta.env.VITE_API_KEY}/internship/${internship.id}`, { certificates: { user: user?.username, codeId: codeId, courseName: internship.name } }, {
          headers: {
          ...
          .then((res) => {
            axios.put(`${import.meta.env.VITE_API_KEY}/user`, { username: user?.username, certificates: { internship: internship?._id, generated: true, codeId: codeId, percentage: 100, name: internship.name, duration: internship.duration, date: new Date().toJSON().slice(0, 10) } }, {
              headers: {
                "Authorization": user?.token,
                "Content-Type": "application/json"
              }
  ...
  const submitQuiz = () => {
    ...
    if (newPoints >= test.length * 75 / 100) {
      ...
      axios.put(`${import.meta.env.VITE_API_KEY}/internship/${internship.id}`, { certificates: { user: user?.username, codeId: codeId, courseName: internship.name } }, {
        ...
        .then((res) => {
          axios.put(`${import.meta.env.VITE_API_KEY}/user`, { username: user?.username, certificates: { internship: internship?._id, generated: true, codeId: codeId, percentage: newPoints / test.length * 100, name: internship.name, duration: internship.duration, date: new Date().toJSON().slice(0, 10) } }, {
            headers: {
              "Authorization": user?.token,
              "Content-Type": "application/json"
            }
